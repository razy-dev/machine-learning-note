...
stem:[
\begin{align}
    input   &= (10, 4) = (timesteps, in\_features) \\
            &= \begin{bmatrix}
            ...
            \end{bmatrix} \\
    \\
    Wx   &= (8, 4) = (hidden\_size, in\_features) \\
            &= \begin{bmatrix}
            ...
            \end{bmatrix} \\
    \\
    Wh   &= (8, 8) = (hidden\_size, hidden\_size) \\
            &= \begin{bmatrix}
    ...
    h_t &= Wx \times x + Wh \times h_{t-1} + b \\
        ...
        &= (8, 1) + (8, 1) + (8, 1) \\
        &= (8, 1) \\
        &= Wx(n, k) \times x(k,1) + Wh(n, n) \times h(n, 1) + b(n, 1) \\
        &= (n, 1) + (n, 1) + (n, 1) \\
        &= (n, 1) \\
\end{align}
]
