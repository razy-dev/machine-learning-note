...
class LogisticRegression(nn.Module):
    def __init__(self, in_features: int, out_features: int, bias: bool = True, activation: str = 'Sigmoid'):
    ...
    @classmethod
    def activation(cls, activation: str):
        try:
...
model = LogisticRegression(2, 1, activation='LeakyReLU')
# end::hypothesis[]
...
# tag::run[]
def runner(logic: str, epochs: int = 1000, threshold: float = 0.2):
    print(f"\nFor '{(logic or '').upper()}'")
    x_train, y_train, x_test, y_test = dataset(logic)
    train(x_train, y_train, epochs, threshold)
